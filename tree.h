...
private:
    ...
    bool (*cmpFunc)(T, T);                      //функция сравнения двух узлов

public:
    //конструкторы
    Tree() : root(nullptr)
    {
        cmpFunc = [](T left, T right)->bool{
            return (left < right);
        };
    }
    ...
    Tree(bool(*cmp)(T,T)) : root(nullptr), cmpFunc(cmp)
    {}
    ...
    Tree(const Tree<T>& src) : root(nullptr), cmpFunc(src.cmpFunc)
    {
