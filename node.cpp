...
template<typename T>
Node<T>* Node<T>::add_node_by_cmp_func(T data, bool(*cmpFunc)(T, T))
{
    Node<T>* root;
    ...
    if(cmpFunc(nodeData, data) != -1)
    {
        left = left->add_node_by_cmp_func(data, cmpFunc);
        left->update_height();
        left->parent = this;
    }
    else
    {
        right = right->add_node_by_cmp_func(data, cmpFunc);
        right->update_height();
        right->parent = this;
    }
